...
agent = A2C(state_shape,action_shape,actor_lr=0.8, critic_lr=0.8, gamma=0.99)
...
for i in range(max_episode):
    ...
    while done == False:
        ...
        step_num += 1
        action_final = agent.select_action(observation)
        print("Action after reset ",action_final)
        print("\n*********************")
        observation, reward, done = env.step(action_final)
        print('Reward:',reward,"\n",'Episode:', i,"\n", 'Step:',step_num,"\n","Curr high eps reward:",curr_highest_eps_reward,'\n', 'saved:',save_count,'\n', 'cutoff count:', cutoff_count,'\n','done ?',done,'\n')

    observation, eps_reward, done = env.step(action)
